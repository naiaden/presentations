...
\note[itemize]{
    ...
    \item The BNP approach is an alternative to parametric modelling and selection.
    \item Bayesian methods are by no means immune to overfitting, but provide a natural resilience
    \item By using a model with an unbounded complexity, underfitting is migitated, and computing, or approximating, the full posterior over parameters migitates overfitting.

    \item NP does not mean there are no parameters
    \item not parametric, the numbers of parameters is not fixed once and for all
    \item So it's not opposed to parameters, to the contrary: the framework can be viewed as allowing an infinite number of parameters, or rather of finite but unbounded dimension
}
...
\note[itemize]{
    ...
    \item The DP is a distribution over probability measures, which are functions with certain special properties which allow them to be interpreted as distributions over some probabily space $\Theta$. A draw from a DP can thus be interpreted as random distributions.
    \item Called DP because it has Dir. distri. finite-dimensional marginal distributions
    
    \item A standard Bay.\ model for clustering involves assuming each data point is assigned to one of $K$ clusters
    \item The probability of being in cluster $k$ is $w_k$ for each cluster, and the probability vector is normalised. We place a dirichlet prior on the prob vector $\{w_k\}$

    \item As an infinite-dimensional analog, you would expect to take $K$ to infinity to derive at the DP
    \item But that is not the BNP approach (Infinite Mixture Model): instead we take a combined combinatorial and statistical approach to the clustering problems (DP mixture model)
...
\note[itemize]{
    ...
    \item metaphore of a restaurant, coined by Lester Dubins and Jim Pitman in the early 80's
...
\begin{frame}
    ...
    \begin{block}{Example for $\{\{1,2,5\},\{3,4\},\{6\}\}$}
        ...
        \end{center}            
        \note[item]<1>{1}
        \note[item]<1>{ So, a new customer joins a table $c$, if $c$ is already a cluster, with prob prop to the number of customers already sitting at that table}
        \note[item]<1>{ otherwise starts a new table, with a prob prop to fixed constant $\alpha$}
        \note[item]<1>{ The seating pattern after $N$ customers thus defines a set of clusters}

        \note[item]<1>{ Now for an example, imagine an infinite number of unlabelled tables}
        \note[item]<1>{ when a customer starts a new table, one of the unlabelled tables is chosen arbitrarily}
        \note[item]<1>{ So after $N$ customers have arrived, their seating pattern defines a set of clusters, and thus a partition.}
        \note[item]<1>{ For each value of $N$, we have a different CRP, and thus a different distribution, making the CRP a family of distributions}
        \note[item]<2>{2}
        \note[item]<3>{3}
        \note[item]<4>{4}
        \note[item]<5>{5}
        ...
        $\uncover<3->{P=\frac{\alpha}{\alpha}\uncover<5->{\frac{1}{\alpha+1}\uncover<7->{\frac{\alpha}{\alpha+2}\uncover<9->{\frac{1}{\alpha+3}\uncover<11->{\frac{2}{\alpha+4}\uncover<13->{\frac{\alpha}{\alpha+5}$}}}}}} \uncover<14->{$\rightarrow P(\pi_{[N]}) = \frac{\alpha^K}{\alpha^{(N)}}\prod_{c\in\pi_{[N]}}(|c|-1)!$}
    \end{block}
\end{frame}
\note[itemize]{

    \item As you can see people don't have anything to eat here yet.
    \item The sequence $x_1, x_2,\ldots,x_6$ as generated by the CRP can be shown to be exchangeable. That is, the points are invariant to permuting the order of the sequence. De Finetti's theorom then shows that there must be a distribution over distributions, such that the points are conditionally independent and iid draws. The DP is one such distribution here.
    \item .
    \item a direct way to construct a weighted sum of an infinite sequence of point masses (with probability one) is the stick-breaking construction, also called the GEM distribution.
}
...
\begin{frame}
    ...
    We want to beat a MKN approach with skipgrams: Pickhardt et al., \textit{A Generalized Language Model as the Combination of Skipped n-grams and Modified Kneser Ney Smoothing}, ACL'14.
...
\end{frame}
\note[itemize]{
    \item Unfortunately we don't have our competitor's system running
    \item only results that we generated ourselves

    \item JRC 31M 8M
    \item 1BW 10\% 80M 8M (4gram, 90G, 11days)
    \item EMEA 12M 3M
}
...
\end{frame}
\note[itemize]{
    \item As expected, the first thing that catches the eye, is the good performance on the diagonals
    \item Second, our performance is similar to cpyp. It should be, because the only thing that differs is the BOS and EOS marker
    \item We outperform traditional MKN by a fair margin

    \item skipgrams results?
}
...
\end{frame}
\note[itemize]{
    \item We compared two backoff methods
    \begin{description}
        \item[ngram] only backoffs to the patterns in the model. Although perhaps confusing, in the lower example the model is trained on skipgrams as well, and there it backoffs to skipgrams of a strict lower order as well
        \item[glm] backoffs to all possible patterns, $n$-grams and skipgrams
    \end{description}

    \item ngram appears to be better within domain, whereas glm seems to be better cross domain
}
...
\note[itemize]{
    ...
    \item $\alpha$ is the concentration parameters, or inverse variance. Larger $\alpha$, the smaller the variance, and more mass concentrated around mean
...
\begin{frame}
    \frametitle{This is a dice}
\scalebox{6}{\begin{tikzpicture}[diceface/.style={diamond, inner sep=3pt, shape aspect=2,
rounded corners=3pt, scale=2}]
% Corner at (0,0)
\fill[gray!10!white] (-0.3, -0.3) rectangle (0.3, 0.3);
% TOP (one)
\fill (0,0) node[fill=white, diceface, anchor=south, draw=white] (topface) {};
\fill[red, yscale=0.7] (topface.center) circle(4pt);
% RIGHT (three)
\fill (0,0) node[fill=gray!15!white, diceface, anchor=south, rotate=-120, draw=white] (rightface) {};
\fill (rightface.center) circle(2pt);
\fill (rightface.center)+(0.15,0.2) circle(2pt);
\fill (rightface.center)+(-0.15,-0.2) circle(2pt);

% LEFT (two)
\fill (0,0) node[fill=gray!30!white, diceface, anchor=south, rotate=120,draw=white] (leftface) {};
\fill (leftface.center)+(0.13,0.1) circle(2pt);
\fill (leftface.center)+(-0.13,-0.1) circle(2pt);
\end{tikzpicture}}
\end{frame}
\note{
